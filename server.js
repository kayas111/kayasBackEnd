...
function CreditNewKayasers (PositionOfLastNewKayaser,amount){
 db.collection('kayasers').find().toArray().then(resp=>{
let kayasersArray=resp, caseKayasers=kayasersArray.reverse().slice(0,(kayasersArray.length-PositionOfLastNewKayaser)+1)
  let contacts=caseKayasers.map(kayaser=>kayaser.contact)
  db.collection('traders').updateMany({ contact: { $in: contacts } },
    { $inc: { accBal: amount } }).then(resp=>{
      console.log(`Increased balance of ${resp.modifiedCount} contacts by ${amount}`)
    })
...
if(payment.paymentApproved==false){
  ...
  res.send({msg:`This ticket has already been confirmed and is now expired. Ticket was bought by ${payment.name} - 0${payment.contact}. .`})
} else{
      ...
      db.collection('traders').updateOne({contact:paymentDetails.beneficiary.contact},{$inc:{'accBal':parseInt(payLoad.data.amount)}}).then(resp=>{
        db.collection('recommendations').find({recommendeeContact:parseInt(paymentDetails.beneficiary.contact)}).toArray().then(resp=>{
          if(resp.length==0){
            ...
            db.collection('traders').updateOne({contact:recommenderContact},{$inc:{'accBal':1000}}).then(resp=>{
